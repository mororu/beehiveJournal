// src/routes/sells/[sellId]/edit/+page.server.ts
...
import { getHoneySaleById, updateHoneySale } from '$lib/server/db/queries/honeySales.js';
...
export const load: PageServerLoad = ({ params }) => {
	const id = parseInt(params.sellId, 10);
	if (isNaN(id)) error(404, 'Verkauf nicht gefunden');
...
export const actions: Actions = {
	default: async ({ request, params }) => {
		...
		const existing = getHoneySaleById(id);
		if (!existing) error(404, 'Verkauf nicht gefunden');
		...
		// field-level checks (see sells/new for rationale).
