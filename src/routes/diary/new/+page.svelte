<script lang="ts">
	...
	let dateInputValue = $state(form?.entryDateRaw || toDateInput(Math.floor(Date.now() / 1000)));
...
<svelte:head>
	<title>Neuer Eintrag — Tagebuch</title>
</svelte:head>
	...
	>
		...
		<div class="field">
			...
			<input
				...
				maxlength="200"
				bind:value={title}
				placeholder="z.B. Frühjahrshonig geerntet"
				required
				disabled={isSubmitting}
			/>
		</div>
		...
		<p class="gps-hint">
			...
			{:else if gpsStatus === 'granted'}
				Wetter wird nach dem Speichern erfasst.
		...
		<div class="form-actions">
			<a href="/diary" class="btn btn--ghost">Abbrechen</a>
