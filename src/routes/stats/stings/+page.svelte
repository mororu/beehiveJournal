<script lang="ts">
	...
	function applyYear(value: string) {
		goto(`/stats/stings?year=${value}`, { replaceState: true, keepFocus: true, noScroll: true });
	}
...
<svelte:head>
	<title>Statistik: Stiche — beehiveJournal</title>
</svelte:head>
	...
	{:else}
		...
		<section class="card">
			...
			<!-- {#key} is load-bearing: the chart has no update path, so without it the
			     component instance survives and keeps the old bars. Keyed on the buckets
			     ARRAY, not on the year: load() returns a fresh array on every run, so this
			     remounts on a year switch AND after invalidateAll(), which offline sting
			     sync calls (src/lib/client/offline/sync.ts) and a year key would miss. -->
